...
CREATE PROCEDURE RegisterMovie(
...
BEGIN
	...
	END;
	SELECT COUNT(*) INTO @movieRegisted FROM Movies WHERE Title = pTitle;
    IF  (@movieRegisted != 0)
    THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'La pelicula ya está registrada', MYSQL_ERRNO = MovieInUSe;
	END IF;
    SET @director = -1 ;
    ...
    START TRANSACTION;
		INSERT INTO Movies (Title,Year,MinAge,Duration,ParticipantId,PictureURL) VALUES (pTitle, pYear, pMinAge, pDuration, @director, imgURL);
	COMMIT;
